...
def compute_content_cost(a_C, a_G):
    ...
    J_content = 1/(4*n_W*n_H*n_C)*tf.reduce_sum((a_C-a_G)**2)
    ### END CODE HERE ###
...
def compute_layer_style_cost(a_S, a_G):
    ...
    a_S = tf.transpose(tf.reshape(a_S,shape=[n_W*n_H,n_C]))
    a_G = tf.transpose(tf.reshape(a_G,shape=[n_W*n_H,n_C]))
    ...
    J_style_layer = 1/(4*(n_C*n_C)*((n_W*n_H)*(n_W*n_H)))*tf.reduce_sum((GS-GG)*(GS-GG))
